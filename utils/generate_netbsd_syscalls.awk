...
function pre_syscall(syscall)
{
  ...
  if (syscall == "syscall") {
    ...
    pcmd("const int *name = (const char *)name_;")
    pcmd("if (name) {")
...
#    pcmd("}")
    ...
    pcmd("struct __sanitizer_timespec **tptr = (struct __sanitizer_timespec **)tptr;")
    pcmd("if (tptr) {")
    pcmd("  PRE_READ(tptr[0], struct_timespec_sz);")
    pcmd("  PRE_READ(tptr[1], struct_timespec_sz);")
    pcmd("}")
    ...
    pcmd("struct __sanitizer_timespec **times = (struct __sanitizer_timespec **)times_;")
    pcmd("if (times) {")
    pcmd("  PRE_READ(times[0], struct_timespec_sz);")
    pcmd("  PRE_READ(times[1], struct_timespec_sz);")
    pcmd("}")
